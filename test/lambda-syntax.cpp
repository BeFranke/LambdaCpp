...
TEST_F(SyntaxTest, VariableParses) {
    is << "x;";
    auto result = std::dynamic_pointer_cast<Variable>(p.statement().last_command().execute());
    ASSERT_EQ(result->get_name(), "x");
}

TEST_F(SyntaxTest, LambdaParsesSimple) {
    is << "\\ x . x;";
    auto result = std::dynamic_pointer_cast<Lambda>(p.statement().last_command().execute());
    os << *result;
...
TEST_F(SyntaxTest, ApplicationParsesSimple) {
    is << "(f) x;";
    auto result = std::dynamic_pointer_cast<Application>(p.statement().last_command().execute());
    os << *result;
